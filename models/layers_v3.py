...
class StaticExpansionBlock(nn.Module):
    def __init__(self, d_model, num_enc_exp_list, dropout_perc, eps):
        ...
        self.class_a_embed = nn.Linear(d_model, d_model*2)
        self.class_a_embed1 = nn.Linear(d_model*2, d_model)

        self.class_b_embed = nn.Linear(d_model, d_model*2)
        self.class_b_embed1 = nn.Linear(d_model*2, d_model)
    ...
    def forward(self, x, n_indexes, mask):
        ...
        class_a = torch.matmul(class_a_fw, self.class_a_embed1(self.class_a_embed(x))) + bias_exp
        class_b = torch.matmul(class_b_fw, self.class_b_embed1(self.class_b_embed(x))) + bias_exp
...
class DynamicExpansionBlock(nn.Module):
    ...
    def forward(self, x, n_indexes, mask):
        ...
        class_a_fw = class_a_fw / (class_a_fw.sum(dim=-1, keepdim=True) + self.eps)
        class_b_fw = class_b_fw / (class_b_fw.sum(dim=-1, keepdim=True) + self.eps)
        class_a = torch.matmul(class_a_fw, self.class_a_embed1(self.class_a_embed(x)))
        class_b = torch.matmul(class_b_fw, self.class_b_embed1(self.class_b_embed(x)))
        class_a = self.dropout_class_a_fw(class_a)
        class_b = self.dropout_class_b_fw(class_b)
